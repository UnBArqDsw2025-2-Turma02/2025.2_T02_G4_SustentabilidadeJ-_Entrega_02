...
![Diagrama de Estados - Login](../assets/Diagramas/Estados/Diagrama_estados_Login.png)

Fonte: elaborado por [Gabriel Lopes](https://github.com/BrzGab)

O diagrama de estados da funcionalidade de **Login e Cadastro** inicia no estado *Abrindo site*, quando o usuário acessa o sistema pela primeira vez. Em seguida, o sistema entra no estado *Solicitando Cadastro/Login*, onde ocorre a decisão: se o usuário já está cadastrado, o fluxo segue para o processo de login; caso contrário, o fluxo vai para o processo de cadastro.

No **subestado Fazendo cadastro**, o usuário inicia em *Inserindo dados de cadastro* e depois passa para *Validando cadastro*. Se os dados forem inválidos, o fluxo retorna para *Inserindo dados de cadastro*. Quando o cadastro é validado com sucesso, o processo é concluído e o usuário fica apto a realizar login.

No **subestado Fazendo login**, o usuário começa em *Inserindo dados de login* e segue para *Validando dados*. Se os dados forem inválidos, o fluxo retorna para *Inserindo dados de login*. Caso os dados sejam válidos, o sistema passa para o estado *Verificando tipo de usuário*, onde ocorre a diferenciação entre **Usuário Comum** e **Administrador**. Usuários comuns são direcionados para a *Página de Missões*, enquanto administradores acessam a *Página de administrador*. Em ambos os casos, o processo é encerrado no estado final, representando o acesso concluído.  
...
| [Gabriel Lopes](https://github.com/BrzGab) | Diagrama de login | Fui responsável por elaborar o diagrama de estados da funcionalidade Login do sistema SustentabilidadeJá. Além da modelagem, participei ativamente da reunião síncrona em que os diagramas foram discutidos, contribuindo na identificação dos estados e transições mais relevantes, garantindo que o fluxo de autenticação estivesse bem representado. Também colaborei na revisão dos diagramas de Marketplace e de Registrar Ação, validando a consistência entre eles.| O trabalho em conjunto foi importante para manter o alinhamento entre os diagramas, pois conseguimos validar decisões de modelagem em tempo real. A padronização proposta pelo Artur foi seguida, o que garantiu consistência visual entre os diagramas, facilitando a leitura e compreensão. Um ponto que pode ser melhorado é o detalhamento de algumas transições condicionais do Login, que poderiam ter ficado mais explícitas para evitar ambiguidades. Ainda assim, o resultado foi satisfatório e bem validado pelo grupo. |
...
[![Assista à gravação](https://img.youtube.com/vi/11qzJorOn50/0.jpg)](https://youtu.be/11qzJorOn50)

Link da reunião em: https://youtu.be/11qzJorOn50
...
| 1.1    | Adicionando análise crítica de Artur Mendonça, diagrama de estados do marketplace, vídeo da gravação, e metodologia |  [Artur Mendonça Arruda](https://github.com/ArtyMend07) | 21/09/2025 |  | |
| 1.2    | Adicionando análise crítica, diagrama de estados de login |  [Gabriel Lopes](https://github.com/BrzGab) | 21/09/2025 |  | |
